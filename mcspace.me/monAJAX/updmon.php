...
<link rel="stylesheet" href="<?php echo $config['dir']?>template/monStyle.css?<?php echo date('YmdHis');?>" type="text/css">
...
var Monitoring = {
    ...
    createMon: function(){ // Создание основы мониторинга
		...
		for(var i=0, len=servers.length; i<len; i++){// Шаблон каждого сервера
			...
			newHTML += "<?php echo tplRead('server.php')?>"
		};
		newHTML += "<?php echo tplRead('common.php')?>";// Шаблон общего онлайна
            ...
            complete: function(){// После получения данных и их парсинга вносим в мониторинг
                setTimeout(function(){
					...
					$('#Monitoring .OneServer').each(function(i){
						...
						$(this).find('.SrvPing').html('Пинг:&nbsp;'+pingTime[i]+'&nbsp;мс');
