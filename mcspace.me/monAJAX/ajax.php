...
$comm = json_decode(file_get_contents('common.json'), true);// Получаем прошлые общие данные
...
{// Дата для рекордов
    ...
    $v = array('января', 'февраля', 'марта', 'апреля', 'мая', 'июня',
...
{// Class written by xPaw & modded by book777
    ...
    function getp($address, $timeout = 3)
    {
        ...
        exec('ping -c 2 -i 0.2 '.$address, $serverping);
        $mcping = substr($serverping[1],-8,5);
        $ping = $mcping*1000;
        if (!$in) return array(
            'address' => $address,
            'ping' => $ping,
    ...
    private function GetChallenge()
    {
        ...
        if ($data['status'] != null)
            return array(
                'status' => 'Не удалось подтвердить соединение'
            );
