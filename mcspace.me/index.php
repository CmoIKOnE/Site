...
$page = $_GET['page'];
$file = $dir . $page . ".php";
if($_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] == "mcspace.me/") $file = $dir . "index.php";
...
<body>
    <div class="wrapper">
    	<?php
    	if ($_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] == "mcspace.me/") {
    		$file = "/";
    		echo "<div class='bg'>
			<div class='container'>";			
    		...
    		echo "<div class='bg-other-all'>";
			require_once "include/header_other.php";
			echo "<div class='bg-other'>";
			require_once $file;
			echo "</div></div>";
			require_once "include/footer_other.php";
		}
    	?>	
    </div>
</body>
...
$(document).ready(function() {
	...
	$('#hider-auth').click(function() {
		var el = $('#auth');
		el.hasClass('auth_hidden') ? (el.addClass('auth_visible'), el.removeClass('auth_hidden')) : (el.addClass('auth_hidden'), el.removeClass('auth_visible'));
	});
	...
	// получение данных из полей
		...
		if (passWord > 7 && passWord < 25) {
			...
			document.getElementById("passimg").src='../img/ico_bad.png';
			document.getElementById('passerrmsg').innerHTML = 'Пароль должен быть от 8 до 24 знаков';
		}
